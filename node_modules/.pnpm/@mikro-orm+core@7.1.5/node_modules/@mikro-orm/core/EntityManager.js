...
export class EntityManager {
     ...
     * ```
     */
    async clone(entityNameOrEntity, whereOrOverrides, overridesOrOptions, options) {
        const em = this.getContext(false);
        let entityName;
        let where;
        let overrides;
        if (Utils.isEntity(entityNameOrEntity)) {
            ...
            entityName = entityNameOrEntity.constructor;
            where = helper(entityNameOrEntity).getPrimaryKey();
            overrides = whereOrOverrides;
            options = overridesOrOptions;
        }
        else {
            // clone(EntityClass, where, overrides?, options?)
            entityName = entityNameOrEntity;
            where = whereOrOverrides;
